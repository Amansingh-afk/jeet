...
async function generatePatternEmbeddings() {
  ...
  const result = await query<{
    id: string;
    name: string;
    signature: { structure: string; keywords: string[]; variations?: string[]; embedding_text?: string };
  }>(`
...
async function generateQuestionEmbeddings() {
  ...
  const whereClause = mode === 'all' ? '' : 'WHERE embedding IS NULL';
  const result = await query<{ id: string; text_en: string }>(`
    SELECT id, text_en
    FROM questions
    ${whereClause}
  `);
...
async function createVectorIndexes() {
  ...
  try {
    ...
    const patternCount = await query<{ count: string }>(
      "SELECT COUNT(*) as count FROM patterns WHERE embedding IS NOT NULL"
    );
    ...
    const questionCount = await query<{ count: string }>(
      "SELECT COUNT(*) as count FROM questions WHERE embedding IS NOT NULL"
    );
...
async function main() {
  ...
  if (skipPatterns) console.log('Skipping patterns');
  if (skipQuestions) console.log('Skipping questions');
