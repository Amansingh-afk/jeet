...
async function seedPatterns() {
    ...
    for (const topicDir of topicDirs) {
        ...
        for (const patternFile of patternFiles) {
            const pattern = readJsonFile(path.join(patternsDir, patternFile));
            if (!pattern)
                continue;
            await patternRepository.upsert({
                ...
                prerequisites: (pattern.prerequisites || { patterns: [], concepts: [] }),
                difficulty: pattern.metadata?.difficulty || 2,
                frequency: (pattern.metadata?.frequency || 'medium'),
...
async function seedQuestions() {
    ...
    for (const topicDir of topicDirs) {
        ...
        for (const questionFile of questionFiles) {
            const question = readJsonFile(path.join(questionsDir, questionFile));
            if (!question)
                continue;
            await questionRepository.upsert({
                ...
                extracted_values: question.extracted_values,
                solution: question.solution,
                source: question.source,
                exam_history: (question.exam_history || []),
...
async function seedTemplates() {
    ...
    for (const templateFile of templateFiles) {
        const template = readJsonFile(path.join(templatesDir, templateFile));
        if (!template)
            continue;
